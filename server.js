...
const NAME_MAX = 12;
const HOST_TOKEN = process.env.HOST_TOKEN || null; // si se define, sustituye la detección por IP local
...
wss.on('connection', (ws, req) => {
  ...
  const isHost = HOST_TOKEN
    ? new URL(req.url, 'http://x').searchParams.get('key') === HOST_TOKEN
